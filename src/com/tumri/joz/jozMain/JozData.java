...
*/
	...
	throws Exception
    {
	String guid = IFASLUtils.toString (iter.next (), "guid");
	String catalog = IFASLUtils.toString (iter.next (), "catalog");
	String product_id = IFASLUtils.toString (iter.next (), "product id");
	ArrayList<String> parents = new ArrayList<String> ();
	parents.add (IFASLUtils.toString (iter.next (), "parents"));
	Float price = IFASLUtils.toFloat (iter.next (), "price");
	Float discount_price = IFASLUtils.toFloat (iter.next (), "discount price");
	String brand = IFASLUtils.toString (iter.next (), "brand");
	String retailer = IFASLUtils.toString (iter.next (), "retailer");
...
// end rank temp hack 
	...
	Integer image_width = IFASLUtils.toInteger (iter.next (), "image width");
	Integer image_height = IFASLUtils.toInteger (iter.next (), "image height");
	Integer cpc = IFASLUtils.toInteger (iter.next (), "cpc");
	String currency = IFASLUtils.toString (iter.next (), "currency");
	String discount_currency = IFASLUtils.toString (iter.next (), "discount currency");
	Integer black_white_list_status = IFASLUtils.toInteger (iter.next (), "black white list status");
	String product_type = IFASLUtils.toString (iter.next (), "product type");
	Float cpo_cpa = IFASLUtils.toFloat (iter.next (), "cpo/cpa");
	...
	try
	{
		...
		// Methinks I'd rather use StringTokenizer here but the docs
		// say its use is discouraged.  Blech.
		String[] tokens = s.split (String.valueOf (delim));
		if (tokens.length != 4)
		    throw new BadMUPDataException ("taxonomy entry needs 4 elements: "
						   + s);
		String id = tokens[0]; // E.g. TUMRI_14385
		String name = tokens[1]; // E.g. Action Figures
