...
public class CmdTSpecDelete extends CommandDeferWriting {
    ...
    public Sexp process() {
        ...
        try {
            if (!expr.isSexpList())
                return SexpReader
                        .readFromStringNoex("(:error \"expected (t-spec-delete :name name)\")");
    ...
    private Sexp delete_tspec(SexpList rqst) throws BadCommandException {
        if (rqst.size() != 3 || !rqst.get(1).isSexpKeyword()
