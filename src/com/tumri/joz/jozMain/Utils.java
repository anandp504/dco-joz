...
public class Utils
{
    ...
    static int
    skipNonWhitespace (String line, int idx)
    {
	while (idx < line.length ()
	       && ! Character.isWhitespace (line.charAt (idx)))
	    ++idx;
    ...
    /**
      The delimiter may not be \, it is used to prefix delimiters
    ...
    */
    ...
    static int
    extractDelimitedString (String line, int idx)
	throws Exception
    {
	int startIdx = idx;
	...
	while (true)
	{
		...
		String msg = "extractDelimitedString: delimited string prematurely terminated, line ["
		    + line + "], started at " + startIdx + ", delimiter " + delimiter;
		log.error (msg);
		throw new Exception (msg);
	    }
