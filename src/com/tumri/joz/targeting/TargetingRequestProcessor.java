...
//				}
		...
		if (handle != null) {
			...
			boolean isCTROpt = theCampaign.isOptimizeCTR();
			if (theCampaign != null) {
			...
			if (theAdPod != null) {
				...
				try {
					targetingResults = selectCreative(request, theAdPod, isCTROpt, feature);
	...
	//todo: change to use theAdPod.getAdPodExperienceLocationMappings() instead of theAdPod.getExperienceId();

	private TargetingResults selectCreative(AdDataRequest request, AdPod theAdPod, boolean isCTROpt, Features feature) throws VectorSelectionException {
		VectorTargetingProcessor proc = VectorTargetingProcessor.getInstance();
		...
		if (recipes != null || expId < 0) {
			...
			exp = ExperienceVectorTargetingProcessor.getInstance().processRequest(adpodId, request, isCTROpt, feature);
