...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	  ...
	  <title>Joz Console : WM Content Status</title>
  ...
  <body>
	<%
        ...
        WMContentProviderStatus status = null;
        String errorMessage = "";
        try {
            if ((requestValue != null) && requestValue.equals("reload")) {
                WMDBLoader.forceLoadData();
            }
            status = WMContentProviderStatus.getInstance();
        }
        catch (Exception ex) {
	...
	<div>
         <% if (status == null) { %>
              Encountered internal error when displaying wm content. Details : <%=errorMessage%>
         <% } else { %>
      <table class="table">
      <tr class="table_header">
          <td colspan=2>WM Data Refresh Summary</td>
      </tr>
      <tr>
          <td>WM refresh interval:</td>
      ...
      <tr>
        ...
        <br>
        <p class="label">
            <a href="http://<%=request.getServerName()%>:<%=request.getServerPort()%>/joz/jsp/wm-content-status.jsp?reload" onclick="return confirm('This will force a wm data refresh. Do you want to continue?');">Click to force Reload</a>
        </p>
        <br>
        <% if (status.runHistory != null && status.runHistory.size() > 0) { %>
         <table class="table">
          ...
          <tr class="table_column_header">
           ...
           <% for (WMContentProviderStatus.WMContentProviderStatusHistory hist : status.runHistory) { %>
           <tr class="table_row">
