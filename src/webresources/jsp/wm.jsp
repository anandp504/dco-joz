...
<table class="table" border="1">
	...
	<tr valign="middle">
		<td align="left" rowspan="<%=size +1%>">
			<%=adPod.getId()%>, <a href="/joz/jsp/adPodSelection.jsp?selAdPod=<%=adPod.getId()%>"><%=adPod.getName()%></a>
		</td>
		<td align="center">
			Id, Type, Value
		</td>
		...
		if(iter!=null){
			while (iter.hasNext()) {
				...
				for (WMIndex.Attribute k: keys) {
					...
					%><tr><%
					%><td><%=contextId %>,<%=k.name()%>,<%=val%></td><%
					List<RecipeWeight> rwList = h.getRecipeList();
		            %><td><ul><%
					for (RecipeWeight rw: rwList) {
						Recipe r = campaignDB.getRecipe(rw.getRecipeId());
						%>
						<li><%=r.getId()%>,<a href="/joz/jsp/recipeSelection.jsp?selRecipe=<%=r.getId()%>"><%=r.getName()%></a>,<%=rw.getWeight()%></li>
						<%
					}
					%></ul></td></tr><%
				}
